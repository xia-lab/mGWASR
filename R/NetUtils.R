...
DecomposeGraph <- function(gObj, minNodeNum = 2){
  ...
  for(i in 1:length(comps)){
    ...
    if(vcount(g) > 0){
      ...
      mSetObj$dataSet$type.nums = c(mSetObj$dataSet$type.nums, my.stat$node.num)
      mSetObj$dataSet$query.nums = c(mSetObj$dataSet$query.nums, my.stat$query.num);
      net.stats[i,] <- c(
        vcount(g),
        ecount(g),
...
convertIgraph2JSON <- function(g, filenm){
  ...
  node.sizes <- as.numeric(rescale2NewRange((log(node.dgr))^2, minSize, 8));
...
GetNetStatByType <- function(g){
  ...
  my.stat <- list(
    query.num = sum(sd.queries %in% nd.queries),
